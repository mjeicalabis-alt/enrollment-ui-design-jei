...
function App() {
  ...
  const handleChange = (e) => {
    ...
    if (["mobile","landline","zip","gsYear","jhsYear","shsYear","shsAverage"].includes(name)) {
      updatedValue = numbersOnly(value);
    }

    if (name === "mobile" && updatedValue && updatedValue.length !== 11)
      newErrors.mobile = "Mobile must be 11 digits.";

    if (name === "landline" && updatedValue && updatedValue.length !== 10)
      newErrors.landline = "Landline must be 10 digits.";

    if (name === "zip" && updatedValue && updatedValue.length !== 4)
      newErrors.zip = "Zip must be 4 digits.";

    if (["gsYear","jhsYear","shsYear"].includes(name) && updatedValue) {
      if (Number(updatedValue) < 1900 || Number(updatedValue) > 2026)
        newErrors[name] = "Enter valid year (1900–2026).";
    }

    if (name === "shsAverage" && updatedValue) {
      if (Number(updatedValue) < 0 || Number(updatedValue) > 100)
  ...
  if (form.level === "Undergraduate") {
    programList = undergraduatePrograms[form.department] || [];
  } else if (form.level === "Graduate") {
    programList = graduatePrograms[form.department] || [];
  }
    ...
    <div className="container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <fieldset>
            ...
            {["firstName","middleName","lastName","suffix"].map(field => (
              <div key={field}>
                ...
                <input
                  ...
                  className={getBorderClass(field)}
          ...
          <input type="text" name="religion" placeholder="Religion" value={form.religion} onChange={handleChange} className={getBorderClass("religion")} />
        </fieldset>
        ...
        <fieldset>
          ...
          <input type="email" name="email" placeholder="Email Address" value={form.email} onChange={handleChange} className={getBorderClass("email")} />
          <label>Mobile Number</label>
          <input type="text" name="mobile" maxLength="11" placeholder="Mobile Number" value={form.mobile} onChange={handleChange} className={getBorderClass("mobile")} />
          <label>Landline Number</label>
          <input type="text" name="landline" maxLength="10" placeholder="Landline Number" value={form.landline} onChange={handleChange} className={getBorderClass("landline")} />
            ...
            {["street","barangay","city","province","zip"].map(field => (
              <div key={field}>
                ...
                <input
                  ...
                  placeholder={field === "zip" ? "Zip Code" : ""}
                  value={form[field]}
                  onChange={handleChange}
                  className={getBorderClass(field)}
                />
        ...
        <fieldset>
          ...
          <div className="grid-4">
            ...
            <input type="text" name="shsAverage" placeholder="SHS Average" value={form.shsAverage} onChange={handleChange} className={getBorderClass("shsAverage")} />
        ...
        <fieldset>
          ...
          <div className="radio-group">
            ...
            <div className="radio-options">
              <label><input type="radio" name="level" value="Undergraduate" checked={form.level==="Undergraduate"} onChange={handleChange}/> Undergraduate</label>
              <label><input type="radio" name="level" value="Graduate" checked={form.level==="Graduate"} onChange={handleChange}/> Graduate</label>
            </div>
          </div>
          ...
          <div className="radio-group">
            ...
            <div className="radio-options">
              <label><input type="radio" name="semester" value="1st Semester" checked={form.semester==="1st Semester"} onChange={handleChange}/> 1st Semester</label>
              <label><input type="radio" name="semester" value="2nd Semester" checked={form.semester==="2nd Semester"} onChange={handleChange}/> 2nd Semester</label>
            </div>
          </div>
          ...
          <div className="radio-group">
            ...
            <div className="radio-options">
              <label><input type="radio" name="campus" value="Manila" checked={form.campus==="Manila"} onChange={handleChange}/> Manila</label>
              <label><input type="radio" name="campus" value="Quezon City" checked={form.campus==="Quezon City"} onChange={handleChange}/> Quezon City</label>
            </div>
          </div>
          ...
          {form.level && (
            <>
              ...
              <select name="department" value={form.department} onChange={handleChange} className={getBorderClass("department")}>
                <option value="">Select Department</option>
                {Object.keys(form.level==="Undergraduate"?undergraduatePrograms:graduatePrograms).map(dept=>(<option key={dept}>{dept}</option>))}
              </select>
            </>
          )}
          ...
          {form.department && (
            <>
              ...
              <select name="degree" value={form.degree} onChange={handleChange} className={getBorderClass("degree")}>
                <option value="">Select Degree Program</option>
                {programList.map(prog=>(<option key={prog}>{prog}</option>))}
              </select>
            </>
          )}
        </fieldset>
